...
MODEL_NAME = 'gemini-1.5-flash' # Updated to valid 1.5 flash model
...
def save_history(username, meal_desc, result_dict):
    ...
    history[username].insert(0, entry) # Prepend newest
...
def analyze_meal(input_data, is_image=False):
    ...
    Strict Rules:
    1. Calculate the total calories and macros.
    2. Account for hidden calories typical in Indian cooking (e.g., ghee, oil, coconut).
    3. Output ONLY a valid JSON object. Do not include markdown formatting, backticks, or conversational text.
...
def inject_css() -> None:
    st.markdown(
        ...
        :root {
            ...
            --border: #4A3A2C; --turmeric: #E7B23A; --chili: #D65B3E; --cardamom: #7FAE6F;
        }
        ...
        .result-calories { font-family: 'JetBrains Mono', monospace; font-weight: 700; font-size: 2.6rem; line-height: 1.1; margin: 0.4rem 0 1.1rem 0; }
        .result-calories span { font-size: 1rem; font-weight: 500; color: var(--muted); margin-left: 0.3rem; }
        
        .macro-row { display: flex; gap: 1.8rem; padding-top: 1rem; border-top: 1px solid var(--border); flex-wrap: wrap; }
        .macro { display: flex; flex-direction: column; gap: 0.15rem; }
        .macro-label { font-size: 0.78rem; color: var(--muted); }
        .macro-value { font-family: 'JetBrains Mono', monospace; font-weight: 600; font-size: 1.05rem; }
        ...
        .result-notes { margin-top: 1rem; font-size: 0.85rem; color: var(--muted); font-style: italic; white-space: pre-line; }
        </style>
        """,
...
def login_screen():
    ...
    with st.container():
        ...
        user = st.text_input("Username", placeholder="e.g. aditi99")
...
def main(): 
    ...
    with st.sidebar:
        ...
        if not user_history:
            ...
            for item in user_history[:5]: # Show last 5 entries
                ...
                st.caption(f"{item['timestamp']} • {item['result'].get('calories', 0)} kcal")
                st.divider()
    ...
    with tab1:
        ...
        if camera_photo and st.button("Calculate", key="btn_scan", type="primary"):
            ...
            with st.spinner("Analyzing spices..."):
